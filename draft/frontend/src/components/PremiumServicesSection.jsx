...
import { BarChart3, Network, Code2, ArrowRight, CheckCircle, Sparkles, Zap } from 'lucide-react';
  ...
  return (
    <section id="services" className="py-24 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-gradient-to-r from-emerald-400/10 to-teal-400/10 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
          {mockData.services.map((service, index) => (
            <GlassmorphismCard 
              key={service.id}
              className={`bg-gradient-to-br ${getGradientColors(index)} border-white/30`}
            >
              <GlassmorphismCardHeader className="pb-6">
                <div className="flex items-center justify-between mb-6">
                  <div className={`relative p-4 bg-gradient-to-br from-white/80 to-white/60 rounded-2xl text-indigo-600 group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                    <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/20 to-emerald-500/20 rounded-2xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity"></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <div>
                ...
                <div className="space-y-6">
                  {[
                    { 
                      text: "Expertise reconnue en WebFOCUS et Business Intelligence",
                      icon: "🏆"
                    },
                    { 
                      text: "Approche sur mesure adaptée à vos enjeux métier",
                      icon: "🎯"
                    },
                    { 
                      text: "Équipe certifiée et expérimentée",
                      icon: "👨‍💻"
                    },
                    { 
                      text: "Accompagnement de A à Z avec support continu",
                      icon: "🤝"
                    },
                    { 
                      text: "ROI mesurable et performances optimisées",
                      icon: "📈"
                    }
                  ].map((point, idx) => (
                    <div key={idx} className="flex items-start bg-white/60 rounded-xl p-4 backdrop-blur-sm">
                      <span className="text-2xl mr-4 mt-1">{point.icon}</span>
                      <span className="text-gray-700 text-lg font-medium leading-relaxed">{point.text}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="text-center">
                <GlassmorphismCard className="bg-gradient-to-br from-white/90 to-white/70 border-white/50 inline-block">
                  <div className="p-12">
                    <div className="text-7xl font-black bg-gradient-to-r from-indigo-600 to-emerald-600 bg-clip-text text-transparent mb-4">
                      98%
                    </div>
